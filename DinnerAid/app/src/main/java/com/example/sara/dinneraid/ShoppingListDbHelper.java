 ...
 *
 * This implementation is state-ful and very much not thread safe. This is because all the read- and write-methods on the heldper needs the state of the mDb
...
public class ShoppingListDbHelper extends SQLiteOpenHelper {
    ...
    private static final java.lang.String SQL_DELETE_LINE_ROWS =
            "DELETE FROM "+ShoppingListContract.ShoppingListLine.TABLE_NAME;
    ...
    private static  final String SQL_WHERE_DROP_DONE_LINES = ShoppingListContract.ShoppingListLine.COLUMN_NAME_ISDONE +" != 0";
     ...
     */
    public void dropLineTable(SQLiteDatabase db) {
     ...
     * @param whereClause the where clause to add. Omit the 'WHERE' statement. Leave null if all lines are to be dropped.
     */
    public void dropLinesFromTable(@Nullable String whereClause) {
        String whereAppendix = (whereClause == null) ? "" : " WHERE "+whereClause;
        String s = SQL_DELETE_LINE_ROWS + whereAppendix;
        mDb.execSQL(s);
    }
    ...
    /**
     * A method for calling insert() the otherwise private databse.
    ...
    /**
     * close the db relation. make sure to all this to allow garbage collection properly.
     */
    public void closeDb() {
        mDb.close();
    }
     ...
     */
    public void dropCompletedShoppingLines() {
        dropLinesFromTable(SQL_WHERE_DROP_DONE_LINES);
    }

    /**
     * Emptying of the database table for shopping list lines.
     */
    public void dropAllShoppingLines() {
        dropLinesFromTable(null);
    }
     ...
     * @param category A string stating the category of the buy. Has to be input validated beforehand.
     */
    public void addShoppingLine(@NonNull String content, @NonNull String category) {
    ...
    public class LineFilterer implements FilterQueryProvider {
        ...
        public Cursor runQuery(CharSequence constraint) {
            ...
            StringBuilder sb = new StringBuilder(ShoppingListContract.ShoppingListLine.COLUMN_NAME_CATEGORY+" in (");
            sb.append(constraint.subSequence(1,constraint.length()).toString());
            sb.append(") ");

            if (constraint.charAt(0) == '1') {

                sb.append("and "+ShoppingListContract.ShoppingListLine.COLUMN_NAME_ISDONE+" == 0");
            }
     ...
     */
    public Cursor getLines(@Nullable String whereClause) {
